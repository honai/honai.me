...
CSSスクロールスナップはCSSの機能の1つで、縦・横にスクロールする要素に対してスナップ点（スクロール終了の位置が吸い付く点）を導入することができます。この機能を用いて、横にスクロールをすることでスライド画像に順にスナップされるようなカルーセルを実装することができます。HTMLとCSSのベースは次のようになります。
...
### カルーセルを使いやすくするUIの追加実装

CSSスクロールスナップを利用したカルーセルは、それだけでもスライド閲覧としての機能を果たせます。しかし、モバイルデバイスでの操作や、共有の機能を考え、以下のようなUIも実装しました。

- スライドへのリンク
- 前後のスライドへ移動できるボタン
- ページに素早くジャンプするためのスライダー

記事が長くなってしまったので、今回は解説しませんが、また別記事で補足できたらいいなと思っています。
カルーセルの実装はGitHubの [このファイル](https://github.com/honai/honai.me/blob/main/src/_includes/components/SlideCarousel/index.jsx)、
ナビゲーション（ボタンやスライダー、リンク）は [このファイル](https://github.com/honai/honai.me/blob/main/src/js/slide-nav.mjs)

### iframeによる埋め込みページの作成
要件「別のページにスライドを埋め込める」を実現するために、iframeでの埋め込み用ページを作成します。
といっても、前節で説明したカルーセルをビューポートいっぱいに表示するようなページを作成すれば良いでしょう。
