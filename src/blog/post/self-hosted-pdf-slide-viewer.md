...
このように横スクロールでスライドを閲覧でき、**スライド上のハイパーリンクをクリックできる**（スライド2枚目参照）ようになっています。
...
CSSスクロールスナップを利用したカルーセルは、それだけでもスライド閲覧としての機能を果たせます。しかし、モバイルデバイスでの操作や、共有の機能を考えると、以下のようなUIがあると便利です。
...
カルーセルのパーツができたので、あとはこのパーツを再利用して、スライド単体のページとは別に埋め込み用ページを作成します。といっても特に複雑なことはありません。

- ビューポートいっぱいにカルーセルを表示するページを作る
- 埋め込み用のiframeのコードをコピーできるようにする
